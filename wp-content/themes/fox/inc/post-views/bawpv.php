...
add_action('widgets_init', 'WP_Register_Most_Viewed');
function WP_Register_Most_Viewed(){
...
class WP_Widget_Most_Viewed_Posts extends WP_Widget {
	...
	function __construct() {
		$widget_ops = array('classname' => 'widget_most_viewed_entries', 'description' => __( 'The most viewed posts on your site', 'wi' ) );
		parent::__construct('most-viewed-posts', __( 'Most Viewed Posts', 'wi' ), $widget_ops);
	...
	function widget($args, $instance) {
        ...
        $layout = $instance['layout'] == 'small' ? 'small' : 'big';
        ...
        <?php /* ================       START POPULAR HTML        ================ */?>
		<div class="<?php echo esc_attr('wi-popular popular-' . $layout);?>">
        <?php $i = 0; ?>
        ...
        ?>
            ...
            <div class="popular-thumbnail-container">
                <?php wi_display_thumbnail('thumbnail-medium','popular-thumbnail',true,true); ?>
                <span class="popular-counter"><?php printf('%02d',$i);?></span>
                <?php if ($instance['show']) echo '<span class="view-count">' . $count . ' views</span>';?>
            </div><!-- .thumbnail-container -->
            
            <h3 class="popular-title"><a href="<?php the_permalink();?>"><?php the_title();?></a></h3>
            
            <div class="popular-content">
                <p><?php echo wi_subword(get_the_excerpt(),0,20);?> &hellip; <a href="<?php the_permalink();?>" class="readmore"><?php _e('More','wi');?></a></p>
            </div><!-- .popular-content -->
            
        <?php endwhile; ?>
		
		</div><!-- .wi-popular -->
        <?php /* ================       END POPULAR HTML        ================ */?>
	...
	function update( $new_instance, $old_instance ) {
        ...
        $instance['layout'] = $new_instance['layout'] == 'small' ? 'small' : 'big';
	...
	function form( $instance ) {
        ...
        if( isset( $instance['layout'] ) )
			...
			$layout = 'big';
?>
		<p><label for="<?php echo esc_attr($this->get_field_id('title')); ?>"><?php _e( 'Title:', 'wi' ); ?></label>
		<input class="widefat" id="<?php echo esc_attr($this->get_field_id('title')); ?>" name="<?php echo esc_attr($this->get_field_name('title')); ?>" type="text" value="<?php echo esc_attr($title); ?>" /></p>

		<p><label for="<?php echo esc_attr($this->get_field_id('number')); ?>"><?php _e( 'How many posts:', 'wi' ); ?></label>
		<input id="<?php echo esc_attr($this->get_field_id('number')); ?>" name="<?php echo esc_attr($this->get_field_name('number')); ?>" type="text" value="<?php echo esc_attr($number); ?>" size="3" /></p>

		<p><label for="<?php echo esc_attr($this->get_field_id('time')); ?>"><?php _e( 'Which top do you want:', 'wi' ); ?></label>
		...
		<p><label for="<?php echo esc_attr($this->get_field_id('author')); ?>"><?php _e( 'Top for this author only:', 'wi' ); ?></label>
		<select id="<?php echo esc_attr($this->get_field_id('author')); ?>" name="<?php echo esc_attr($this->get_field_name('author')); ?>">
			<option value=""><?php _e( 'All authors', 'wi' ); ?></option>
		<?php foreach( get_users() as $u ) { ?>
			<option value="<?php echo esc_attr($u->ID); ?>" <?php selected( $author_id, $u->ID ); ?>><?php echo ucwords( esc_html( $u->display_name ) ); ?></option>
			<?php } ?>
		</select>
		<?php /* /// soon
		<p><label for="<?php echo $this->get_field_id('author'); ?>"><?php _e( 'Exclude categories: (Multiple choise possible)', 'wi' ); ?></label>
		...
		*/
        /*  /// DONT UNDERSTAND THIS OPTION
		<p><label for="<?php echo $this->get_field_id('date'); ?>"><?php _e( 'Date format', 'wi' ); ?> <code>YYYYMMAA</code></label>
		...
		<code><?php _e( 'If you leave blank the actual time will be used.', 'wi' ); ?></code></p>
        */?>

		<p><label for="<?php echo $this->get_field_id('show'); ?>"><?php _e( 'Show view count:', 'wi' ); ?></label>
        ...
        <?php /* /// THIS MUST BE REDUNDANT =.=
		<p><label for="<?php echo $this->get_field_id('order'); ?>"><?php _e( 'Order', 'wi' ); ?></label>
		<select id="<?php echo $this->get_field_id('order'); ?>" name="<?php echo $this->get_field_name('order'); ?>">
			<option value="DESC" <?php selected( $order, 'DESC' ); ?>><?php _e( 'From most viewed to less viewed', 'wi' ); ?></option>
			<option value="ASC" <?php selected( $order, 'ASC' ); ?>><?php _e( 'From less viewed to most viewed', 'wi' ); ?></option>
		</select>
		</p>
        */
        /*  /// NEXT UPDATE
        <p><label for="<?php echo $this->get_field_id('layout'); ?>"><?php _e( 'Layout', 'wi' ); ?></label>
		<select id="<?php echo $this->get_field_id('layout'); ?>" name="<?php echo $this->get_field_name('layout'); ?>">
            <option value="big" <?php selected( $layout, 'big' ); ?>><?php _e( 'Big', 'wi' ); ?></option>
			<option value="small" <?php selected( $layout, 'small' ); ?>><?php _e( 'Small', 'wi' ); ?></option>
		</select>
		</p>
        */?>
<?php
...
{
	add_settings_section( 'bawpvc_settings_page', sprintf( __( 'General', 'wi' ), BAWPVC_FULLNAME ), '__return_false', 'bawpvc_settings' );
		add_settings_field( 'bawpvc_field_time_count', __( 'Time between counts', 'wi' ), 'bawpvc_field_time_count', 'bawpvc_settings', 'bawpvc_settings_page' );
		add_settings_field( 'bawpvc_field_format', __( 'Count format', 'wi' ), 'bawpvc_field_format', 'bawpvc_settings', 'bawpvc_settings_page' );
		add_settings_field( 'bawpvc_field_in_content', __( 'Display counter in post content', 'wi' ), 'bawpvc_field_in_content', 'bawpvc_settings', 'bawpvc_settings_page' );
		add_settings_field( 'bawpvc_field_no_members', __( 'Do not count views for logged members', 'wi' ), 'bawpvc_field_no_members', 'bawpvc_settings', 'bawpvc_settings_page' );
		add_settings_field( 'bawpvc_field_no_admins', __( 'Do not count views for admins', 'wi' ), 'bawpvc_field_no_admins', 'bawpvc_settings', 'bawpvc_settings_page' );
		add_settings_field( 'bawpvc_field_post_types', __( 'Select post types', 'wi' ), 'bawpvc_field_post_types', 'bawpvc_settings', 'bawpvc_settings_page' );
	add_settings_section( 'bawpvc_settings_page', sprintf( __( 'About', 'wi' ), BAWPVC_FULLNAME ), '__return_false', 'bawpvc_settings2' );
?>
	<div class="wrap">
...
function bawpvc_field_post_types()
{
	...
	foreach( get_post_types( array( ), 'objects' ) as $cpt ):
		...
		$public = $cpt->public ? __( 'Public', 'wi' ) : __( 'Not public', 'wi' );
		printf( '<label><input type="checkbox" %s name="bawpvc_options[post_types][]" value="%s" /> %s <em>(%s)</em></label><br />', $checked, $name, $label, $public );
	endforeach;
	?>
	<code>
	<?php _e( 'Public: Whether a post type is intended to be used publicly either via the admin interface or by front-end users.<br />Not public: The opposite...', 'wi' ); ?>
	</code>
	<?php
}
...
function bawpvc_field_no_members()
{
	global $bawpvc_options;
?>
	<label><input type="checkbox" name="bawpvc_options[no_members]" <?php checked( $bawpvc_options['no_members'], 'on' ); ?> /> <em><?php _e( 'Check me to avoid counting views for logged members.', 'wi' ); ?></em></label>
<?php
}
...
function bawpvc_field_no_admins()
{
	global $bawpvc_options;
?>
	<label><input type="checkbox" name="bawpvc_options[no_admins]" <?php checked( $bawpvc_options['no_admins'], 'on' ); ?> /> <em><?php _e( 'Check me to avoid counting views for admins.', 'wi' ); ?></em></label>
<?php
}
...
function bawpvc_field_time_count()
{
	global $bawpvc_options;
?>
	<input type="number" size="3" maxlength="3" name="bawpvc_options[time]" value="<?php echo absint( $bawpvc_options['time'] ); ?>" /> <em><?php _e( 'seconds', 'wi' ); ?></em>
<?php
}
...
function bawpvc_field_format()
{
	global $bawpvc_options;
?>
	<input type="text" name="bawpvc_options[format]" size="40" value="<?php echo esc_attr( $bawpvc_options['format'] ); ?>" /> <em><?php _e( 'Use <code>%count%</code> to display the counter.', 'wi' ); ?></em>
<?php
}
...
function bawpvc_field_in_content()
{
	global $bawpvc_options;
?>
	<label><input type="checkbox" name="bawpvc_options[in_content]" <?php checked( $bawpvc_options['in_content'], 'on' ); ?> /> <em><?php _e( 'Will be displayed in bottom of content.', 'wi' ); ?></em></label>
<?php
}
...
?>
	<div id="bawpvc_box">
		<?php _e( '(Clic value to edit it)', 'wi' ); ?>
		<br />
		<ul>
			<?php
			global $post, $timings;
				...
				if( current_user_can( $capa, $post->ID ) )
					printf( '<li>' . __( 
								'%1$s: <span class="hide-if-no-js toggle_views" onclick="jQuery(this).hide().next(\'span\').show().find(\'input:visible\').select();" title="%4$s">%2$d</span>'.
											...
											'</span> views', 'wi' .
							'</li>', 'wi' ),
							esc_html( $time ), (int)$count, $date, __( 'Clic to edit me!', 'wi' )
						);
				// else
					// printf( '<li>' . __( '%s: %d views', 'wi' ) . '</li>', esc_html( $time ), (int)$count );
			endforeach;
			?><br />
			<label><input type="checkbox" name="bawpvc_reset" value="on" /> <?php _e( 'Check me to reset all views', 'wi' ); ?></label>
...
{
	$capa = apply_filters( 'baw_count_views_capa_role', 'edit_posts' );
	if( $column_name == 'wi' && current_user_can( $capa ) )
