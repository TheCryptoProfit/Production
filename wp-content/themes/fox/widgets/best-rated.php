<?php
/**
* Best Rated Posts Widget
*/
add_action('widgets_init', 'wi_best_rated_posts_load_widgets');

function wi_best_rated_posts_load_widgets()
{
	register_widget('WI_Widget_Best_Rated');
}

class WI_Widget_Best_Rated extends WP_Widget {
	
	function __construct() {
		$widget_ops = array('classname' => 'widget_best_rated', 'description' => __('Display best rated posts','wi'));
		$control_ops = array('width' => 250, 'height' => 350);
		parent::__construct('best-rated', __('Wi:Best Rated Posts','wi'), $widget_ops, $control_ops);
	}
        ...
        $average = number_format((float)$average, 1, '.', '');
				...
				<li class="rated-article format-<?php echo get_post_format() ? get_post_format() : 'standard';?>">
                    ...
                    <section class="rated-article-content">
                        <h3 class="rated-article-title"><a href="<?php the_permalink();?>"><?php the_title();?></a></h3>
	...
	function form( $instance ) {
		...
		<p>
			<label for="<?php echo esc_attr($this->get_field_id('category')); ?>"><?php _e('Display posts from a category?','wi') ?></label>
