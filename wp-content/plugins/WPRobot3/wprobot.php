...
*/
/*  Copyright 2009 - 2014 Thomas Hoefter
*/
...
$wpr_modules = array("amazon","article","articlebuilder","clickbank","ebay","flickr","yahoonews","youtube","rss","translation","commissionjunction","oodle","pressrelease","shopzilla","itunes","linkshare","eventful","yelp","shareasale","avantlink","plr","bigcontentsearch","kontentmachine");

$wpr_file   = basename( __FILE__ );
$wpr_folder = basename( dirname( __FILE__ ) );
...
function wpr_add_pages_toadmin() {
...
add_action('admin_menu', 'wpr_add_pages_toadmin');
...
function wpr_install_new_modules($newmodules,$options) {
	...
	if ( function_exists('curl_init') ) {
		$request = WPR_URLPATH."/wprinstall2.php";
	...
	if(is_array($installed_modules)) { 
		foreach($wpr_loadedmodules as $module) {
			if(!in_array($module, $installed_modules)) {
				if($module != "translation" && $module != "articlebuilder" && $module != "bigcontentsearch" && $module != "kontentmachine") {
					$newmodules[] = $module;
				}
			}
		}
...
function wpr_install_function($email) {
	...
	if($email == "demo" || $email == "Demo" || $email == "DEMO") {
		...
		if($result) {
			...
			<br/><br/>For instructions take a look at this <a href="http://wprobot.net/documentation/quickstart.php" target="_blank">quick start guide</a> or the <a href="http://wprobot.net/documentation/" target="_blank">online documentation</a>!', 'wprobot');
		} else {
			if (is_multisite()) { 
				echo '<div class="updated"><p>'.__('Error: WP Robot could not be installed. Please note "Network Activate" will not work for WP Robot. You need to activate it on each subblog individually to complete the installation.', 'wprobot').'</p></div>';				
			} else {
...
//echo $response;
		...
		if( $response == "false" || !$response) {
			...
			if($result) {
				...
				$optionsurl = get_option('siteurl') . '/wp-admin/options-general.php';   
				$cronurl = WPR_URLPATH . 'cron.php?code='.get_option("wpr_cron");
				echo '<div class="wrap"><h2>WP Robot</h2><div class="updated" style="line-height:18px;">'.__('<h3 style="margin: 5px 0 0 0;">Success!</h3><br/><b>Thank you for installing WP Robot 4!</b>.
				<br/><br/>For instructions take a look at this <a href="http://wprobot.net/documentation/quickstart.php" target="_blank">quick start guide</a> or the <a href="http://wprobot.net/documentation/" target="_blank">online documentation</a>!', 'wprobot');
				echo "<br/><br/>";
			} else {
				if (is_multisite()) { 
...
function wpr_install_message() {
	...
	if(isset($_POST['wpr_install_message'])) {
		...
		//} else {
			wpr_install_function("ddd");
		//}
	} else {
		echo '<div class="wrap"><h2>WP Robot</h2><div class="updated"><h3>Installation</h3><p>'.__('Please click the "Finish Install" button below to complete the installation of WP Robot and setup your templates.', 'wprobot').'</p>
		<form method="post" id="wpr_install">	
		<!--<strong>Paypal Email:</strong> <input id="wpr_install_email" class="regular-text" type="text" value="ddd" name="wpr_install_email"/>-->
		<input class="button-primary" type="submit" name="wpr_install_message" value="'.__('Finish Install', 'wprobot').'" />
		</form>			
		<br/>
		</div>
		</div>';
...
function wpr_cc($core) {
	...
	elseif($core == "developer") {_e('You are using a <b>Developer Version</b> of WP Robot. This means you can sell this website and include WP Robot with the sale.', 'wprobot');}	
	...
	printf(__('You have %1$s modules of %2$s available modules installed. You can <a href="http://wprobot.net/order/">get new modules</a> here.', 'wprobot'), $lcount, $mcount);
*/
}
		...
		foreach ($comments as $comment) {
			if($i < $cnum) {
				...
				if(function_exists("preg_split")) {
					...
					list( $today_year, $today_month, $today_day, $hour, $minute, $second ) = explode( '([^0-9])', $comment_date );
				}	
	...
	if($translation["chance"] >= rand(1,100)) {
		if($translation["from"] != "no" && $translation["to1"] != "no") {
			...
			if(!empty($translationcontent["error"]["reason"])) {
				...
				if($options["wpr_trans_titles"] == "yes" && empty($translationtitle["error"]["reason"])) {$title = $translationtitle;}
