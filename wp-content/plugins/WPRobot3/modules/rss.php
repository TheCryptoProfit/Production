...
//echo $sourcelink . "<br><br>";			print_r($link);		
	...
	//echo "<br><br>LOOLOLOL " . $descr . " GAGAGAGA ". $fullcontent. "DDD<br><br>";			
			...
			if($abort == 0 && $allabort == 0 && !wpr_check_unique($uid)) {
				...
				if($fullcontent != "" && $options['wpr_rss_content'] != 'desc') {$content = $fullcontent;} else {$content = $descr;}				
				if ($options['wpr_rss_striplinks']=='yes') {$content = wpr_strip_selected_tags($content, array('a'));}
				...
				$post = str_replace("{fulltext}", $item->fulltext, $post);			
				...
				if(!empty($mediathumbnail)) {$customfield = $mediathumbnail;} elseif(!empty($enclosure)) {$customfield = $enclosure;} else {$customfield = "";}	
					...
					$customfield2 = array();
					$customfield2["rsscontent"] = $content;
