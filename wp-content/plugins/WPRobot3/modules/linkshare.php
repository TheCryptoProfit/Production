...
function wpr_linksharerequest($keyword,$num,$start) {	
    ...
    $request = 'http://productsearch.linksynergy.com/productsearch?token='.$appid.'&keyword="'.urlencode($keyword).'"&max='.$num.'&pagenumber='.$start.'&cat='.urlencode($cat).'&sort='.$sort.'&sorttype=asc'; // The request URL used by the API service
   // $request = 'https://api.rakutenmarketing.com/productsearch/1.0?token='.$appid.'&keyword="'.urlencode($keyword).'"&max='.$num.'&pagenumber='.$start.'&cat="'.urlencode($cat).'"&sort='.$sort.'&sorttype=asc&mid='.urlencode($merchant); // The request URL used by the API service
	
	if(!empty($merchant)) {
		$request .= '&mid='.urlencode($merchant);
	}
