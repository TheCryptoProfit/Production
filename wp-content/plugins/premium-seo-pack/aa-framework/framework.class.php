...
if(class_exists('psp') != true) {
	class psp {
		...
		public $plugin_tables = array('psp_link_builder', 'psp_link_redirect', 'psp_monitor_404', 'psp_web_directories', 'psp_serp_reporter', 'psp_serp_reporter2rank', 'psp_post_planner_cron', 'psp_alexa_rank', 'psp_serprank_website', 'psp_serprank_keyword', 'psp_serprank_mainrank', 'psp_serprank_pagerank');
		...
		public $serp_competitor_idx = array(
			'names' 	=> array(),
			'ids' 		=> array(),
		);
		...
		public function __construct($here = __FILE__)
		{
			...
			$this->buildConfigParams('freamwork-css-files', array(
				...
				'sweetalert' => 'js/sweetalert2/sweetalert2-6.6.8.min.css',
			));
			...
			$this->buildConfigParams('freamwork-js-files', array(
				...
				'tipsy'	=> 'js/tooltip.js',
				'sweetalert'	=> 'js/sweetalert2/sweetalert2-6.6.8.min.js',
				...
				'moment' => 'js/chartjs/moment-min-2.13.0.js',
				'chart-bundle' => 'js/chartjs/chart-bundle-2.7.0.js',
				//'chart-plugin-datalabels' => 'js/chartjs/chart-plugin-datalabels-min-0.1.0.js',
				'utils' => 'js/chartjs/utils.js',
				'analyser' => 'js/chartjs/analyser.js',
		...
		*/
		public function admin_get_scripts() {
			...
			$_site_admin_url = @end(explode( '/', rtrim( admin_url(), "/" ) ));
			$current_url = explode($_site_admin_url . "/", $current_url);
			
			if( count($current_url) > 1 ){ 
				$current_url = "/" . ( $_site_admin_url ) . "/" . $current_url[1];
			}else{
				$current_url = "/" . ( $_site_admin_url ) . "/" . $current_url[0];
			}
			...
			foreach( $this->cfg['modules'] as $alias => $module ){
				...
				if( isset($module[$alias]["load_in"]['backend']) && is_array($module[$alias]["load_in"]['backend']) && count($module[$alias]["load_in"]['backend']) > 0 ){
					...
					foreach ( $module[$alias]["load_in"]['backend'] as $page ) {
						...
						$urlfound = preg_match("%^/" . ( $_site_admin_url ) . "/".preg_quote($page)."%", $current_url);
						if(
							// $current_url == '/wp-admin/' . $page
							( ( $page == '@all' ) || ( $current_url == '/' . ( $_site_admin_url ) . '/admin.php?page=psp' ) || ( !empty($page) && $urlfound > 0 ) )
					...
					if( $status == true && isset( $settings[$alias]['module_init'] ) ){
						if( is_file($module_folder . $settings[$alias]['module_init']) ){
							//if( $this->is_admin ) {
								...
								$GLOBALS['psp_current_module'] = $current_module;
		...
		*/
		...
		public function edit_post_inline_data( $post_id, $seo=null, $tax=false, $post_content='empty' ) {
			...
			if ( $this->tax_istax__( $tax ) ) { //taxonomy data!
				...
				$psp_current_taxseo = $this->tax_get_post_meta__( null, $post );
	...
	<table class="psp-inline-edit-post form-table" style="border: 1px solid #dadada;">
	    ...
	    public function tax_get_post_meta__( $post_meta=null, $post=null, $key='' ) {
	    	if ( !$this->tax_istax__( $post ) )
	    		return null;
	    ...
	    public function tax_update_post_meta__( $post=null, $keyval=array() ) {
	    	if ( !$this->tax_istax__( $post ) )
	    		return false;
	    ...
	    public function tax_get_post__( $post=null, $output='OBJECT', $filter='raw' ) {
			if ( !$this->tax_istax__( $post ) )
	    		return null;
	    ...
	    public function tax_istax__( $post=null ) {
		...
		public function get_psp_meta( $post, $current_taxseo=array() ) {
			if ( $this->tax_istax__($post) ) {
				$psp_meta = $this->tax_get_post_meta__( $current_taxseo, $post, 'psp_meta' );
			}
			else {
		 ...
		 * SERP - aug-sept 2017
		 */
		public function array_first_element( $arr=array() ) {
			$first = reset($arr);
		...
		public function get_ids_from_string( $ids='' ) {
			$__rq2 = array();
			$__rq = explode(',', $ids);
			if (is_array($__rq) && count($__rq)>0) {
				foreach ($__rq as $k=>$v) {
		...
		public function get_your_website() {
			$website = get_site_url();

			$output = $this->clean_website_url( $website );
			return $output;
		}

		public function clean_website_url( $url ) {
			...
			$url = preg_replace("~^www\.~im", "", $url);
			$url = rtrim($url, '/');

			return $url;
		}

		public function serp_competitor_list( $pms=array() ) {
			$pms = array_replace_recursive(array(
				'engine'		=> 'all', //all | google.com | google.ro | ...
			), $pms);
			extract($pms);

			$ret = array();

			$table = $this->db->prefix . 'psp_serprank_website';

			$clause = '';
			if ( 'all' != $engine ) {
				$engine_ = $this->db->_escape($engine);
				$clause = "and a.search_engine = '" . $engine_ . "'";
			}
			$sql = "select a.id, a.website, a.search_engine, a.is_competitor from $table as a where 1=1 $clause order by a.id asc;";
			...
			$your_website = $this->get_your_website();

			$rr = array();
			foreach ($res as $key => $val) {
				$_id = $val->id;
				$_website = $val->website;
				$_se = $val->search_engine;
				$_isc = strtoupper( $val->is_competitor );
				...
				// split in competitors & your websites
				if ( 'Y' == $_isc ) {
					$rr["$_se"]['competitors']["$_id"] = $val;
				}
				else {
					$rr["$_se"]['yours']["$_id"] = $val;
				}

				// first found as your website
				if ( empty($rr["$_se"]['you']) && ( ( $your_website == $_website ) || ( 'Y' == $_isc ) ) ) {
					$rr["$_se"]['you']["$_id"] = $val;
				}
				else {
					$rr["$_se"]['you_not']["$_id"] = $val;
				}

				//$first = reset($yours); $first = array_slice($yours, 0, 1); $first = array_shift($first);
			} // end foreach
			$ret = $rr;

			if ( 'all' != $engine && isset($ret["$engine"]) ) {
				$ret = $ret["$engine"];
			}
			return $ret;
		}

		public function serp_competitor_idx_set( $pms=array() ) {
			$pms = array_replace_recursive(array(
				'competitors' 	=> array(
					'names' 		=> array(),
					'ids' 			=> array(),
			...
			$this->serp_competitor_idx = $competitors;

			if ( ! isset($_SESSION['psp_serp']) ) {
				$_SESSION['psp_serp'] = array();
			}
			$_SESSION['psp_serp']['competitor_idx'] = $this->serp_competitor_idx;
		}

		public function serp_competitor_idx_get() {
			//return $this->serp_competitor_idx;
			if ( isset($_SESSION['psp_serp']['competitor_idx']) ) {
				return $_SESSION['psp_serp']['competitor_idx'];
			}
			return array();
		}

		public function serp_competitor_colors( $idx='all', $palette=1 ) {
			$ret = array(
				// lighter colors: used as 'background-color'
				0 => array(
					0 	=> '#CFD8DC',
					1 	=> '#CE93D8',
					2 	=> '#B39DDB',
			...
			$palette = (int) $palette;
			$ret = isset($ret["$palette"]) ? $ret["$palette"] : $ret[0];
			...
			$idx = (int) $idx;
			$ret = isset($ret["$idx"]) ? $ret["$idx"] : '#fff';
			return $ret;
		}

		public function serp_build_score_html( $score=0, $pms=array() ) {
			$pms = array_replace_recursive(array(
				'_display' 			=> 'default', // default | stats
				'show_score'		=> true,
				'css_style'			=> '',
				'score_html' 		=> $score . '%',
			), $pms);
			...
			$main_css = '';
			if ( 'stats' == $_display ) {
				$main_css = ' psp-progress-stats';
			}

			$html = array();
			$html[] = '<div class="psp-progress' . $main_css . '"' . $_css_style . '>';
			$html[] = 		'<div class="psp-progress-bar ' . ( $size_class ) . '" style="width:' . ( $score ) . '%"></div>';
			if ( $show_score ) {
				$html[] =	'<div class="psp-progress-score">' . ( $score_html ) . '</div>';
			}
			$html[] = '</div>';
			return implode('', $html);
		}

		public function serp_build_column_rank( $rank=array() ) {
			$rank = array_replace_recursive( array(
				 // html | array
				'_return' 			=> 'html',

				// default (use font icons) | email (use images as icons)
				'_display' 			=> 'default',

				// exclude showing number of pages - in main interface rank column
				'_exclude_nbpages' 	=> false,
				//'website_nbpages' 	=> 0,

				// DESC: 1 is best, 100 is wors | ASC: 1 is worst, 100 is best
				// because score is backward as keyword position
				'_direction' 		=> 'desc',

				// what do we compare: position | score
				'_compare' 			=> 'position', 
			...
			$main_css = '';
			if ( 'email' == $_display ) {
				$main_css .= ' psp-serp-rankcolumn-email';
			}

			if ( 'email' == $_display ) {
				$_rank_yes = '<span class="the_pos' . $main_css . '" style="font-size: 14px;">%s</span>';
				$_rank_no = '<span class="the_pos' . $main_css . '" style="font-size: 14px;">-</span>';
			}
			else {
				$_rank_yes = '<span class="the_pos">%s</span>'; 
				$_rank_no = '<span class="the_pos">-</span>';
			}
			...
			if ( empty($rank) ) {
				...
				return 'html' == $_return ? $ret['html'] : $ret;
			}
			if ( ! $_exclude_nbpages && ! isset($rank['website_nbpages']) ) {
				$ret = array_replace_recursive($ret, array(
					'html' 	=> $_rank_no,
				));
				return 'html' == $_return ? $ret['html'] : $ret;
			...
			$nb_pages = isset($rank['website_nbpages']) ? (int) $rank['website_nbpages'] : 0;

			$position = array(
				'current' 	=> $rank['position'],
				'prev' 		=> $rank['position_prev'],
			);
			if ( 'position' === $_compare ) {
				$position['current'] = (int) $rank['position'];
				$position['prev'] = (int) $rank['position_prev'];
			}

			if ( 'position' === $_compare ) {
				$position['current_show'] = 999 === $position['current'] ? '+100' : $position['current'];
				$position['prev_show'] = 999 === $position['prev'] ? '+100' : $position['prev'];
			}
			else if ( 'score' === $_compare ) {
				$position['current_show'] = $position['current'] . '%';
				$position['prev_show'] = $position['prev'] . '%s';
			}

			if ( -1 === $position['current'] ) {
				$html[] = $_rank_no;
			}
			else if ( -1 === $position['prev'] ) {
				$html[] = sprintf( $_rank_yes, $position['current_show'] );
			}
			else {

				if ( 'score' === $_compare ) {
					$position['current'] = (float) $rank['position'];
					$position['prev'] = (float) $rank['position_prev'];
				}

				// direction how
				$moved = 'same';
				if ( $position['current'] > $position['prev'] ) {
					$moved = 'desc' == $_direction ? 'down' : 'up';
				}
				else if ( $position['current'] < $position['prev'] ) {
					$moved = 'desc' == $_direction ? 'up' : 'down';
				}
				$position['moved'] = $moved;

				//direction icon!
				if ( 'email' == $_display ) {
					$icon = '<img class="" style="" src="{{images_base_url_gen}}images/icon_{dir}.png" alt="{dir}" />%s';
					$icon = str_replace( '{dir}', $position['moved'], $icon );
				}
				else {
					$icon_class = array(
						'same' 	=> 'psp-checks-arrows-h',
						'up' 	=> 'psp-checks-arrow-up',
						'down' 	=> 'psp-checks-arrow-down',
					);
					$icon = '<i class="{dir}">%s</i>';
					$icon = str_replace( '{dir}', $icon_class["{$position['moved']}"], $icon );
				}

				//evolution difference asc | desc
				if ( 'position' === $_compare ) {
					$position['diff'] = (999 === $position['current'] ? 101 : $position['current']);
					$position['diff'] -= (999 === $position['prev'] ? 101 : $position['prev']);
				}
				else if ( 'score' === $_compare ) {
					$position['diff'] = $position['current'];
					$position['diff'] -= $position['prev'];
					$position['diff'] = number_format( $position['diff'], 1 );
				}

				$position['showit'] = array();
				$position['showit'][] = $position['current_show'];

				$show_diff = '';
				if ( abs( $position['diff'] ) ) {
					$show_diff = '<span class="{dir}"{style}>{diff}{proc}</span>';
					$show_diff = str_replace( '{dir}', $position['moved'], $show_diff );
					$show_diff = str_replace( '{diff}', abs( $position['diff'] ), $show_diff );

					if ( 'email' == $_display ) {
						$__style = ' style="font-size: 0.9rem; vertical-align: auto; display: inline-block; margin-top: 0rem; color: {color};"';
						$__style = str_replace( '{color}', ( 'up' == $position['moved'] ? '#7cc012' : '#e71212' ), $__style );
						$show_diff = str_replace( '{style}', $__style, $show_diff );
					}
					else {
						$show_diff = str_replace( '{style}', '', $show_diff );
					}
				}

				if ( 'score' === $_compare && ! empty($show_diff) ) {
					$show_diff = str_replace( '{proc}', '%', $show_diff );
				}
				else {
					$show_diff = str_replace( '{proc}', '', $show_diff );
				}

				$position['showit'][] = sprintf( $icon, $show_diff );

				$html[] = sprintf( $_rank_yes, implode(PHP_EOL, $position['showit']) );
			...
			if ( ! $_exclude_nbpages && ( $nb_pages > 1 ) ) {
				$html[] = '<div class="nb_pages" title="' . __('number of website pages found in top', 'psp') . '">' . $nb_pages . '</div>';
			}
		...
		public function serp_stats_get_top_types() {
			$tops = array( 1, 3, 5, 10, 30, 50, 100 );
			return $tops;
		}
		
		public function serp_last_check_date( $date=null ) {
			$pms = array(
				// D j M / H.i  OR  D j M Y / H.i
				'format' 	=> 'j M Y',
		...
		// REPORT module - report types
		public function report_get_types( ) {
			$reportsList = array(
				'serp|serp_rank_changes' 		=> array(
					'alias' 		=> 'serp_rank_changes',
					'title'			=> 'Report - SERP Keywords Rank Changes',
					'desc' 			=> 'Report - SERP Keywords Rank Changes'
				),
				'serp|serp_website_stats' 	=> array(
					'alias' 		=> 'serp_website_stats',
					'title' 		=> 'Report - SERP Your Website Stats',
					'desc' 			=> 'Report - SERP Your Website Stats'
				),
			);
			return $reportsList;
		}

		public function print_download_file( $pms=array() ) {
			$pms = array_replace_recursive(array(
            	'content' 		=> '',
                'filename' 		=> '',
			), $pms);
		...
		public function pdf_api_do_request_auto( $pms=array() ) {
			$pms = array_replace_recursive(array(
            	'page_url'		=> '',
            	'page_content' 	=> '',
			), $pms);
			...
			$opStat = $this->pdf_api_get_valid_keys();

			if ( ( 'invalid' == $opStat['status'] ) || empty($opStat['keys']) ) {
				$ret = array_replace_recursive($ret, $opStat, array());
				return $ret;
			}
			$keys = $opStat['keys'];
			...
			$contor = 0;
			do {
				$opStat = $this->pdf_api_do_request( array(
					'api_key' 		=> array_shift( $keys ),
	            	'page_url'		=> $page_url,
	            	'page_content' 	=> $page_content,
				));

				if ( 'valid' == $opStat['status'] ) {
					$ret = array_replace_recursive($ret, $opStat, array());
					return $ret;
				}
				$contor++;
			}
			while( ! empty($keys) && ( $contor <= $max ) );

			$ret = array_replace_recursive($ret, $opStat, array());
			return $ret;
		}

		public function pdf_api_do_request( $pms=array() ) {
			$pms = array_replace_recursive(array(
				'api_key' 		=> '',
            	'page_url'		=> '',
            	'page_content' 	=> '<!DOCTYPE html><title>Test PDF conversion</title>This is the body',
			), $pms);
			extract( $pms );
			...
			$api_stat = get_option('psp_pdf_api', array());
			$db_keys_stat = isset($api_stat['keys']) ? $api_stat['keys'] : array();

			//"http://api.html2pdfrocket.com/pdf?apikey=" . urlencode($api_key) . "&value=" . urlencode($page_content)
			$api_url = "http://api.html2pdfrocket.com/pdf";
			$api_postdata = array(
				'apikey' => $api_key,
				'value' => $page_url,
            ...
            $resp_result = wp_remote_post( $api_url, $api_args );
    		//var_dump('<pre>', $resp_result , '</pre>'); echo __FILE__ . ":" . __LINE__;die . PHP_EOL;
			if ( is_wp_error( $resp_result ) ) { // If there's error
				//$err = htmlspecialchars( implode(';', $resp->get_error_messages()) );
				return false;
			}
			$resp_body = wp_remote_retrieve_body( $resp_result );
			$resp_headers = wp_remote_retrieve_headers( $resp_result );
			//var_dump('<pre>', $resp_headers, $resp_headers['content-type'] , '</pre>'); echo __FILE__ . ":" . __LINE__;die . PHP_EOL;

			if ( isset($resp_headers['content-type']) && ('application/pdf' == $resp_headers['content-type']) ) {
				$ret = array_replace_recursive($ret, array(
					'status' 	=> 'valid',
					'msg' 		=> 'ok!',
				...
				$ret = array_replace_recursive($ret, array(
					'msg' 		=> '"application/pdf" content type header is missing!',
				));

				$tries = isset($db_keys_stat["$api_key"]['error_tries']) ? $db_keys_stat["$api_key"]['error_tries'] : 0;
				$db_keys_stat["$api_key"] = array(
					'error_tries' 	=> ++$tries,
					'timestamp' 	=> time(),
				);
			...
			$api_stat['keys'] = $db_keys_stat;
			update_option('psp_pdf_api', $api_stat);

			//var_dump('<pre>', $ret , '</pre>'); echo __FILE__ . ":" . __LINE__;die . PHP_EOL;
			return $ret;
		}

		public function pdf_api_get_valid_keys() {
			$ret = array(
				'status'	=> 'invalid',
				'msg'		=> '',
				'keys' 		=> array(),
			...
			$ss = isset($ss['html2pdfrocket_key']) ? $ss['html2pdfrocket_key'] : '';
			...
			$keys = array();
			foreach ($db_keys as $keyid) {
				if ( isset($db_keys_stat["$keyid"]) ) {
					$keys["$keyid"] = $db_keys_stat["$keyid"];
				}
				else {
					$keys["$keyid"] = array(
						'error_tries' 	=> 0,
			...
			$keys_valid = array();
			foreach ($keys as $keyid => $keyinfo) {
				// key is valid if no more than 3 error tries were made with it, otherwise becomes invalid and it's blocked for a period
				if ( $keyinfo['error_tries'] < 3 ) {
					$keys_valid[] = $keyid;
				}
				// key is valid if blocked period expired and another 3 tries are allowed
				else if ( ( $keyinfo['timestamp'] + 3600 ) < time() ) {
					$keys_valid[] = $keyid;
				}
			}
			...
			$ret = array_replace_recursive($ret, array(
				'status' 	=> 'valid',
				'msg' 		=> sprintf( '%s valid keys found.', count($keys_valid) ),
				'keys' 		=> $keys_valid,
			));
			return $ret;
		}
