...
pspFreamwork = (function ($) {
	...
	function ajax_list()
	{
		...
		var make_request = function( action, params, callback ){
			...
			var ajax_table 	= params['ajax_table'],
				ajax_id 	= ajax_table.find('.psp-ajax-list-table-id').val();
			//console.log( params['ajax_table'], ajax_id );
			delete params['ajax_table'];
		...
		$(".psp-table-ajax-list").on('change', 'select[name=psp-post-per-page]', function(e){
			...
			var $this       = $(this),
				ajax_table 	= $this.parents('.psp-table-ajax-list:first');
	...
	function init_custom_checkbox()
	{
		$('.psp-main input[type="checkbox"]').each(function() {
			...
			if( !$this.parent().hasClass('psp-custom-checkbox') ) {
				$this.wrap(
					$('<div class="psp-custom-checkbox" />').prop({
						'id' : chk_id + '-wrap'
					})
				);
			}
