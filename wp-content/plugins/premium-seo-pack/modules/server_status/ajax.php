		...
		public function verify_step() {
			...
			if ( $module == 'serp' ) {
				...
				if ( $action == 'step1' ) {
					...
					$return = array(
						'status'	=> $mandatoryValid ? 'valid' : 'invalid',
						'log' 		=> $mandatoryValid ? __('all mandatory fields: developer key, custom search id are set!', 'psp') : __('some of the mandatory fields: developer key, custom search id are NOT set!', 'psp'),
						'execution_time' => number_format( microtime(true) - $start, 2)
					);
				} // end step 1
				...
				if ( $action == 'step2' ) {
					...
					$getGraph = $pspSERP->verify_api_status();
