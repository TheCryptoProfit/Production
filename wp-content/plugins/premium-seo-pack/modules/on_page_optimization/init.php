...
if (class_exists('pspOnPageOptimization') != true) {
    class pspOnPageOptimization
    {
		...
		private function makePrintBoxParams( $pms=array() ) 
		{
			...
			$ret = array(
				'ga'						=> null,
				'__istax'					=> $this->the_plugin->tax_istax__( $tax ),
			...
			// base info!
			if ( $this->the_plugin->tax_istax__( $tax ) ) { //taxonomy data!
				...
				$psp_current_taxseo = $this->the_plugin->tax_get_post_meta__( null, $post );
				if ( is_null($psp_current_taxseo) || !is_array($psp_current_taxseo) )
					$psp_current_taxseo = array();

				$post_seo_status = $this->the_plugin->tax_get_post_meta__( $psp_current_taxseo, $post_id, 'psp_status' );
			...
			if ( $this->the_plugin->tax_istax__( $tax ) ) { //taxonomy data!
				...
				//$focus_kw = $this->the_plugin->tax_get_post_meta__( $psp_current_taxseo, $post, 'psp_kw' );
			...
			//	&& ! $this->the_plugin->tax_istax__( $tax ) ) {
...
<?php
		...
		public function get_seo_report( $id=0, $kw='', $returnAs='die', $data='large' )
		{
			...
			if ( ! $this->the_plugin->tax_istax__( $request['id'] ) ) {
			...
			if(
				...
				// NOTICE: for taxonomy refresh!
				|| $this->the_plugin->tax_istax__( $request['id'] )
			...
			$rules_allowed = $this->the_plugin->get_content_analyzing_allowed_rules( array(
				'settings'	=> array(),
				'istax'		=> $this->the_plugin->tax_istax__( $request['id'] ),
			));
			...
			foreach ( $post_seo_status as $kwitem => $kwinfo ) { // foreach multi keywords
				...
				//if ( $this->the_plugin->tax_istax__( $request['id'] ) ) { //taxonomy data!
		...
		public function optimize_page( $id='', $kw='', $returnAs='die' )
		{
			...
			if ( $this->the_plugin->tax_istax__( $request['id'] ) ) { //taxonomy data!
				...
				$psp_current_taxseo = $this->the_plugin->tax_get_post_meta__( null, $request['id'] );
				if ( is_null($psp_current_taxseo) || !is_array($psp_current_taxseo) )
					$psp_current_taxseo = array();
				
				$post_metas = $this->the_plugin->get_psp_meta( $request['id'], $psp_current_taxseo );
				$post = $this->the_plugin->tax_get_post__( $request['id'], ARRAY_A );
			...
			if( !is_null($post) && count($post) > 0 ) {
				...
				//ajax request from plugin module! - quick edit action
					...
					if ( !$this->the_plugin->tax_istax__( $request['id'] ) ) //not taxonomy data!
				...
				if ( $this->the_plugin->tax_istax__( $request['id'] ) ) { //taxonomy data!
					...
					$this->the_plugin->tax_update_post_meta__( $request['id'], array(
		...
		public function save_seo_score( $p=0, $pms=array() )
		{
			...
			if ( $this->the_plugin->tax_istax__( $p ) ) //taxonomy data!
			...
			if ( $this->the_plugin->tax_istax__( $p ) ) { //taxonomy data!
				$this->the_plugin->tax_update_post_meta__( $p, array(
					'psp_status'	=> $status,
					'psp_score'		=> $score,
					'psp_kw'		=> $kw,
				));
