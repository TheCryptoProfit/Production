        ...
        if ( isset( $data[ 'html_ads' ] ) && !empty( $data[ 'html_ads' ] ) ) {
            foreach ( $data[ 'html_ads' ] as $banner_index => $banner_title ) {
                ...
                if ( in_array( $data[ 'banner_ids' ][ $banner_index ], $newBanners ) ) {
                    $sql         = 'INSERT INTO ' . ADS_TABLE . ' SET campaign_id=%d, meta=%s, type=%d, status=%d, weight=%d, link=%s, banner_custom_js=%s';
                    $params      = array( $new_campaign_id, $meta, $type, $status, $weight, $bannerLink, $bannerCustomJs );
                    $wpdb->query( $wpdb->prepare( $sql, $params ) );
                    $newBannerId = $wpdb->insert_id;
                } else {
                    $bannerId = $data[ 'banner_ids' ][ $banner_index ];
                    $sql      = 'UPDATE ' . ADS_TABLE . ' SET meta=%s, status=%d, weight=%d, link=%s, banner_custom_js=%s WHERE image_id=%d';
                    $params   = array( $meta, $status, $weight, $bannerLink, $bannerCustomJs, $bannerId );
                    $wpdb->query( $wpdb->prepare( $sql, $params ) );
                }
