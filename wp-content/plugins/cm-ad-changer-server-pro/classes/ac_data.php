...
// Scripts in footer
    ...
    public static function get_history_month( $month, $campaign_id = null ) {
        ...
        $month_details = $wpdb->get_results( 'SELECT count(1) as cnt, i.title, i.filename, "impressions" as event_type
											...
											GROUP BY i.title
										UNION
											SELECT count(1) as cnt, i.title, i.filename, "clicks" as event_type
											FROM ' . HISTORY_TABLE . ' h
											INNER JOIN ' . ADS_TABLE . ' i ON h.banner_id=i.image_id
											WHERE DATE_FORMAT(h.regdate,"%M")="' . $month[ 0 ] . '" AND
												  DATE_FORMAT(h.regdate,"%Y")="' . $month[ 1 ] . '" AND
    ...
    public static function get_history_days_data( $date_from, $date_to, $campaign_id = null ) {
        ...
        $range_details_clicks = $wpdb->get_results( 'SELECT count(event_id) AS cnt, date(h.regdate) AS date, i.title, i.filename, h.event_type, i.type, i.meta
                                                    FROM ' . HISTORY_TABLE . ' h
    ...
    public static function getHistoryGroups( $month, $group_id = null ) {
        ...
        $range_details_clicks = $wpdb->get_results( 'SELECT count(event_id) AS cnt, date(h.regdate) AS date, i.title, i.filename, h.event_type
                                                    FROM ' . HISTORY_TABLE . ' h
                                                    JOIN ' . ADS_TABLE . ' i ON h.banner_id=i.image_id
