...
public $consumername;
		...
		public function doTransactionRequest($sIssuerId, $sPurchaseId, $fPurchaseAmount, $sPurchaseDescription, $sEntranceCode, $sReturnUrl, $sCallbackUrl = '', $notifyurl)
		{
			$this->sTransactionId = false;
			$this->sTransactionUrl = false;
			...
			$aGetData['testmode'] = 'true';
			$aGetData['test'] = 'true';
			$aGetData['description'] = $sPurchaseDescription;
			$aGetData['returnurl'] = $sReturnUrl;
			$aGetData['notifyurl'] = $notifyurl;
			$aGetData['notify'] = 'true';
			...
			else // Realtime validation
			{
				...
				if($sXmlReply)
				{
					...
					elseif($this->parseFromXml('transaction', $sXmlReply))
					{
						...
						$consumername = $this->parseFromXml('consumername', $sXmlReply);
						$sConsumerAccount = $this->parseFromXml('consumeraccount', $sXmlReply);
						$sSignature = $this->parseFromXml('sha1', $sXmlReply);
						$this->sConsumerAccount = $sConsumerAccount;
