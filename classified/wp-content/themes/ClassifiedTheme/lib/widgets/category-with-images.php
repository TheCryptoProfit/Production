...
add_action('widgets_init', 'classified_register_browse_by_category_widget_images');
function classified_register_browse_by_category_widget_images() {
	register_widget('classifiedtheme_browse_by_category_with_images');
}

class classifiedtheme_browse_by_category_with_images extends WP_Widget {

	function classifiedtheme_browse_by_category_with_images() {
		...
		$this->WP_Widget( 'browse-by-category-thumbs', 'ClassifiedTheme - Browse by Category Thumbs', $widget_ops, $control_ops );
	}
	...
	function widget($args, $instance) {
		...
		if ( $count > 0 )
		{
			...
			// echo '<style>#'.$widget_id.' .my_image_div_cat_name { width: '.round(100/$nr).'%}</style>';
			 ...
			 {
					$classifiedtheme_get_cat_pic_attached = classifiedtheme_get_cat_pic_attached($term->term_id);
					$link 		= get_term_link($term->slug,	"ad_cat");
					$image 		= classifiedtheme_generate_thumb3($classifiedtheme_get_cat_pic_attached, 'ra_my_category_image_thing');
		...
		else { _e('There are no categories defined.','classifiedtheme'); }
